...
class Solution:
    ...
    def helper(self, arr, i, target, li, ans, s):
        if s > target:
            return
        ...
        li.append(arr[i])
        self.helper(arr, i, target, li, ans, s+arr[i])
        li.pop()
        self.helper(arr, i+1, target, li, ans, s)

    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        ans = []
        li = []
        self.helper(candidates, 0, target, li, ans, 0)
        return ans
